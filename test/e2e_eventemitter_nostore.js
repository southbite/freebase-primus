...
//	We are testing the deletion of data at a set path, and listening for the DELETE event at that path.
  ...
  it('should subscribe to an object and only get the diff', function (done) {
    this.timeout(default_timeout);

    try {

      publisherclient.set('/e2e_test1/testDiff',
        {
        ...
        , null, function (e, result) {
          listenerclient.on('/e2e_test1', 'PUT', 1, function (e, message) {

            expect(message.path).to.be('/e2e_test1');
            expect(message.data).to.eql({testDiff:{property1: {sub_property1: 'new_subp1'}}});
            done(e);
            ...
            if (!e) {
              publisherclient.set('/e2e_test1/testDiff/property/sub_property1', 'new_subp1', null, function (e, result) {
              });
