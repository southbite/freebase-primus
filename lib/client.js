...
/*BEGIN CLIENT-SIDE ADAPTER - DO NOT REMOVE THIS COMMENT
			...
			var http_adapter = {
				perform_request:function(params, done){
					...
					var data = JSON.stringify({encapsulated:encodeURIComponent(JSON.stringify(params.json)), basetype:basetype, client:'jquery'});
			...
			config.url = 'http://' + config.host + ':' + config.port
  			...
  			var request_helper = {
				...
				performRequest:function(url, method, headers, data, done){
					...
					//////////console.log('in client perform req');
					//////////console.log(data);
...
*///END CLIENT-SIDE ADAPTER			
				...
				on:function(path, event_type, count, handler, done){
					...
					try{
						...
						//////console.log([path, event_name, count]);
...
Socket = Primus.createSocket({ "transformer": config.transformer, "parser": config.parser, "manual":true });
//END SERVER-SIDE ADAPTER
			
/*BEGIN CLIENT-SIDE ADAPTER - DO NOT REMOVE THIS COMMENT

$.getScript( config.url + '/events/client.js', function( data, textStatus, jqxhr ) {
...
//BEGIN SERVER-SIDE ADAPTER - DO NOT REMOVE THIS COMMENT
					...
					//////console.log(config.url);
	  				...
	  				client.pubsub = new Socket(config.url);

	  				client.pubsub.on('error', function (err) {
					  console.error('Something horrible has happened', err.stack);
					});

					client.pubsub.on('data', function (data) {
					  ////console.log('received data...');
					  ////console.log(data);

					  if (data.type == 'event'){

					  	if (data.path == '/ALL@all')
					  		client.handle_event(data.path, data);
					  	else
					  		client.handle_event(data.path, data.payload);
