module.exports = {
	initialize:function(config, done){
		...
		try{
			...
			_this.connectionId = _this.pubSubService.connect(_this.handle_publication.bind(_this));
	...
	setInternal:function(path, data, options, handler){
		...
		if (options && options.noStore) return _this.pubSubService.handleDataResponse(null, path, _this.dataService.transformSetData(path, data), options, 'PUT', handler);
