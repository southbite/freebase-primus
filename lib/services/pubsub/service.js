...
module.exports = {
	...
	getListenerDict: function(path) {
		...
		if (path == '/ALL@*') return _this.listeners_all;
		if (path.indexOf('*') > -1) return _this.listeners_wildcard;
		return _this.listeners;
	},
	addListener: function(path, sessionToken) {
		var _this = this;
		//_this.utils.log('addListener', 'trace', 'service-pubsub', arguments);

		var decoded = _this.freebase.services.auth.decodeToken(sessionToken);
		if (path == '/ALL@*') return _this.listeners_all[sessionToken] = 1;
		var listener_dict = _this.getListenerDict(path);
		if (!listener_dict[path]) listener_dict[path] = {};
		listener_dict[path][sessionToken] = 1;
	},
	removeListener: function(path, sessionToken) {
		var _this = this;
		//_this.utils.log('removeListener', 'trace', 'service-pubsub', arguments);
		var listener_dict = _this.getListenerDict(path);
	...
	emitToAudience: function(audience, publication, channel) {
		var _this = this;
		//_this.utils.log('emitToAudience', 'trace', 'service-pubsub', arguments);

		if (audience) {
			publication.channel = channel;
			for (var socketId in audience) {
	...
	publish: function(message, payload) {
		var _this = this;
		//_this.utils.log('publish', 'trace', 'service-pubsub', arguments);
		
        var channel = "/" + message.action.toUpperCase() + '@' + message.path;
        var allActionsChannel = '/ALL@' + message.path;
        ...
        _this.emitToAudience(_this.listeners[allActionsChannel], publication, allActionsChannel);
        _this.emitToAudience(_this.listeners_all, publication, '/ALL@*');

        for (var listenerPath in _this.listeners_wildcard){

            if (wildcard(listenerPath, channel)){
                //console.log('emitting to listener: ' + listenerPath);
                _this.emitToAudience(_this.listeners_wildcard[listenerPath], publication, listenerPath);
            }

            if (wildcard(listenerPath, allActionsChannel)){
                //console.log('emitting to listener: ' + listenerPath);
                _this.emitToAudience(_this.listeners_wildcard[listenerPath], publication, listenerPath);
            }
        }
