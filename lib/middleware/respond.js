...
module.exports = {
	publish:function(req, done){
		...
		_this.freebase.services.pubsub.publish({payload:req.result, path:'/' + req.message.action + '@' + req.message.path, action:req.message.action, params:req.message.params});
		done();
	},
	respond:function(req, res, err){
		...
		if (!err){
			...
			if (['PUT','DELETE'].indexOf(req.method) > -1){
				...
				if (req.message.params && req.message.params.noPublish)
